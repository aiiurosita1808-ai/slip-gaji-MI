...
old_single = """      // Send to Fonnte using URLSearchParams string
...
new_single = """      // Send to Fonnte via backend proxy (avoids browser quirks)
...
old_mass = """        // Send to Fonnte using URLSearchParams string
        const urlencoded = new URLSearchParams();
        urlencoded.append('target', teacher.phone);
        urlencoded.append('message', finalMessage);
        urlencoded.append('schedule', formattedSchedule);
...
new_mass = """        // Send to Fonnte via backend proxy (avoids browser quirks)
        const response = await fetch('/api/send-wa', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
