const fs = require('fs');
const content = fs.readFileSync('src/components/SalarySlip.tsx', 'utf8');
...
newContent = newContent.replace(
  /const totalPenerimaan =[\s\S]*?const newSlip: SalarySlip = {[\s\S]*?totalSalary\n    };/,
  `const newSlip = generateSlipObject(teacher, selectedMonth, selectedYear);`
);

newContent = newContent.replace(
  /const DataRow = \(\{ label, value \}: \{ label: string, value: number \}\) => \([\s\S]*?\);\n/,
  ``
);

newContent = newContent.replace(
  /<div ref=\{slipRef\} className="bg-white shadow-xl rounded-lg p-10 flex flex-col max-w-4xl mx-auto w-full print-container print:shadow-none print:border-none print:p-0 print:m-0 print:max-w-none">[\s\S]*?<div className="flex justify-end mt-4 pr-16 text-center text-sm">\n\s*<div>\n\s*<p className="mb-20">Dibuat Oleh<\/p>\n\s*<p className="border-b border-black w-48 mx-auto"><\/p>\n\s*<\/div>\n\s*<\/div>\n\s*<\/div>/,
  `<div className="mx-auto flex justify-center w-full max-w-4xl print-container">\n          <SlipDocument slip={viewingSlip} ref={slipRef} />\n        </div>`
);


newContent = newContent.replace(
  /<p className="text-sm text-slate-500 mt-0\.5">Buat dan cetak slip gaji untuk guru\.<\/p>/,
...
fs.writeFileSync('src/components/SalarySlip.tsx', newContent);
